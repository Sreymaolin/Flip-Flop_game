(() => {
  ...
  const bestEl = document.getElementById('best') || { textContent: '0' };
  ...
  class Bird {
    ...
    update(dt) {
      ...
      if (birdFrames.length) {
        ...
        if (this.frameTimer >= 4) {
          this.frameTimer = 0;
          this.frameIdx = (this.frameIdx + 1) % birdFrames.length;
        }
      }
    }
    rect() {
    ...
    draw() {
      ...
      if (img) {
        ...
        tctx.drawImage(img, 0, 0, this.w, this.h);
        ctx.drawImage(temp, -this.w/2, -this.h/2);
      } else {
        ctx.fillStyle = '#ffde59';
        ctx.fillRect(-this.w/2, -this.h/2, this.w, this.h);
        ctx.fillStyle = '#ff8c00';
        ctx.fillRect(this.w/2 - 6, -this.h/2 + 4, 10, 6);
      }
      ctx.restore();
    }
  }
  ...
  class Game {
    ...
    loadHighScore() {
      try { return parseInt(localStorage.getItem('fbHigh') || '0', 10); }
      catch { return 0; }
    }
    saveHighScore() {
    ...
    hit() {
      ...
      this.state = 'over';
      msgEl.innerHTML = `<strong style="color:#c00">Game Over</strong>`;
      this.saveHighScore();
    }
    update(dt) {
      ...
      for (const p of this.pipes) {
        if (rectCollide(br, p.topRect()) || rectCollide(br, p.bottomRect())) {
          this.hit(); break;
        }
      ...
      if (this.bird.y + this.bird.h >= GROUND_Y) {
        this.bird.y = GROUND_Y - this.bird.h;
        this.hit();
      }
    ...
    draw() {
      ...
      if (groundImg) {
        ctx.drawImage(groundImg, 0, GROUND_Y - 20, W, 100);
      } else {
  ...
  if (muteBtn) muteBtn.addEventListener('click', e => { e.stopPropagation(); game.toggleMute(); });
  if (restartBtn) restartBtn.addEventListener('click', e => { e.stopPropagation(); game.reset(); });
